...
export const MajorDedicatedPage: React.FC<MajorDedicatedPageProps> = ({
  ...
  return (
    ...
    >
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-4 max-w-3xl">
            <div className="flex flex-wrap items-center gap-3">
              {!major.isActive ? (
                <span className="px-3.5 py-1 rounded-full text-xs font-black bg-emerald-500/20 text-emerald-400 border border-emerald-500/50 flex items-center gap-1.5 shadow-sm">
              ...
              <span className="px-3 py-1 rounded-full text-xs font-black bg-school-cyan/20 text-school-cyan border border-school-cyan/30">
                {major.hoursPerWeek || (major as any).units || '5 יח״ל בגרות'}
              </span>
              <span className="px-3 py-1 rounded-full text-xs font-semibold bg-school-panel text-school-muted border border-school-line">
                {major.targetGrades || (major.division === 'middle_school' ? 'חטיבת ביניים (ז-ט)' : 'חטיבה עליונה (י-יב)')}
              </span>
              {major.category && (
      ...
      {/* Notice Banner when Inactive / Coming Soon */}
      {!major.isActive && (
        <div className="p-4 sm:p-5 rounded-2xl bg-emerald-950/60 border border-emerald-500/50 text-emerald-300 flex items-center gap-4 shadow-lg">
          ...
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-white">
                דף מגמה זו נמצא בעריכה והכנה
              </span>
            </div>
            <p className="text-xs text-emerald-200/90 leading-relaxed">
              פרטי המגמה המלאים, תכנית הלימודים וטפסי הרישום יפורסמו כאן בקרוב לקראת שנת הלימודים.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-school-panel/80 border border-school-line/60 rounded-2xl p-6 flex items-start gap-4 shadow-sm">
          ...
          <div>
            ...
            <p className="text-xs text-school-muted mt-1">{major.hoursPerWeek || (major as any).units || '5 יח״ל מוגברות ואיכותיות'}</p>
          </div>
        </div>
