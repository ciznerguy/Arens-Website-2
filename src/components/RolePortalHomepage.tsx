...
export const RolePortalHomepage: React.FC<RolePortalHomepageProps> = ({
  ...
  // Student division filter ('all' = שש-שנתי, 'junior' = חט"ב נעורים, 'high' = חט"ע עליונה)
  const [studentDivisionFilter, setStudentDivisionFilter] = useState<'all' | 'junior' | 'high'>('all');
  ...
  const pageEntries = (Object.entries(allPagesMap) as [string, InternalPage][]).filter(([url, page]) => {
    ...
    if (role === 'students') {
      const isAudienceMatch = aud.includes('students') || aud.includes('תלמיד') || aud.includes('כללי') || aud.includes('all') || 
             cat.includes('חט') || cat.includes('תלמיד');
      if (!isAudienceMatch) return false;

      if (studentDivisionFilter === 'junior') {
        return page.divisionScope === 'junior' || (page.divisionScope as any) === 'נעורים' ||
          cat.includes('נעורים') || cat.includes('חט"ב') || cat.includes('חטב') || tit.includes('ז\'') || tit.includes('ח\'') || tit.includes('ט\'') || url.includes('%d7%97%d7%98%d7%91');
      }

      if (studentDivisionFilter === 'high') {
        return page.divisionScope === 'high' || (page.divisionScope as any) === 'עליונה' ||
          cat.includes('עליונה') || cat.includes('חט"ע') || cat.includes('חטע') || cat.includes('מגמות') || tit.includes('י\'') || tit.includes('יא\'') || tit.includes('יב\'') || tit.includes('בגרות') || url.includes('%d7%97%d7%98%d7%a2');
      }
  ...
  return (
    <div className="space-y-8 pb-16 animate-fade-in max-w-7xl mx-auto px-4 md:px-8 pt-4">
      ...
      {/* DIVISION FILTER TABS (FOR STUDENTS ROLE) */}
      {role === 'students' && (
        <section className="space-y-4">
          <div className="bg-school-panel/80 border border-school-line/80 rounded-2xl p-2 md:p-3 flex flex-wrap items-center justify-between gap-3 shadow-lg">
            <div className="flex items-center gap-2 px-2">
              <Compass className="w-4 h-4 text-school-cyan shrink-0" />
              <span className="text-xs font-bold text-school-muted">בחירת חטיבה ושכבות לימוד:</span>
            </div>

            <div className="flex items-center gap-1.5 bg-[#0e1626] p-1 rounded-xl border border-school-line/60">
              <button
                onClick={() => {
                  setStudentDivisionFilter('all');
                  setVisibleCount(6);
                }}
                className={`px-3.5 py-1.5 rounded-lg text-xs font-black transition-all cursor-pointer ${
                  studentDivisionFilter === 'all'
                    ? 'bg-gradient-to-r from-school-cyan to-cyan-400 text-slate-950 shadow-md'
                    : 'text-school-muted hover:text-white'
                }`}
              >
                כלל בית הספר (שש-שנתי)
              </button>

              <button
                onClick={() => {
                  setStudentDivisionFilter('junior');
                  setVisibleCount(6);
                }}
                className={`px-3.5 py-1.5 rounded-lg text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${
                  studentDivisionFilter === 'junior'
                    ? 'bg-gradient-to-r from-emerald-400 to-teal-400 text-slate-950 shadow-md'
                    : 'text-school-muted hover:text-white'
                }`}
              >
                <span>חטיבת נעורים (ז'-ט') 🎓</span>
              </button>

              <button
                onClick={() => {
                  setStudentDivisionFilter('high');
                  setVisibleCount(6);
                }}
                className={`px-3.5 py-1.5 rounded-lg text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${
                  studentDivisionFilter === 'high'
                    ? 'bg-gradient-to-r from-school-violet to-purple-400 text-white shadow-md'
                    : 'text-school-muted hover:text-white'
                }`}
              >
                <span>חטיבה עליונה (י'-יב') 🔬</span>
              </button>
            </div>
          </div>

          {/* DEDICATED DIVISION HUBS */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* JUNIOR HIGH HUB CARD */}
            <div className={`p-5 rounded-3xl border transition-all ${
              studentDivisionFilter === 'junior' 
                ? 'bg-gradient-to-br from-emerald-950/40 via-school-panel to-school-bg border-emerald-500/50 shadow-xl' 
                : 'bg-school-panel/60 border-school-line/60 hover:border-emerald-500/30'
            }`}>
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-2xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-black text-sm">
                    ז'-ט'
                  </div>
                  <div>
                    <h3 className="text-base font-black text-white">חטיבת נעורים</h3>
                    <p className="text-[11px] text-school-muted">שכבות ז', ח', ט', מסלולי מצוינות והווי חברתי</p>
                  </div>
                </div>
                <button
                  onClick={() => onNavigateToPage('students-junior')}
                  className="text-[11px] font-black px-2.5 py-1 rounded-lg bg-emerald-500/15 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/30 transition-all cursor-pointer flex items-center gap-1"
                >
                  <span>מרחב החטיבה</span>
              ...
              <div className="grid grid-cols-3 gap-2 pt-1 text-center">
                <button
                  onClick={() => onNavigateToPage("course/%d7%97%d7%98%d7%91/%d7%a9%d7%9b%d7%91%d7%aa-%d7%96-%d7%94%d7%a6%d7%a2%d7%93-%d7%94%d7%a8%d7%90%d7%a9%d7%95%d7%9f-%d7%91%d7%93%d7%a8%d7%9a-%d7%94%d7%97%d7%93%d7%a9%d7%94/")}
                  className="p-2 rounded-xl bg-white/5 hover:bg-emerald-500/15 border border-white/10 hover:border-emerald-500/40 text-xs font-bold text-white transition-all cursor-pointer"
                >
                  <span className="block text-[10px] text-emerald-400 font-normal">שכבת</span>
                  <span>ז' החדשה</span>
                </button>
                <button
                  onClick={() => onNavigateToPage("course/%d7%97%d7%98%d7%91/%d7%a9%d7%9b%d7%91%d7%aa-%d7%97-%d7%a6%d7%9e%d7%99%d7%97%d7%94-%d7%95%d7%94%d7%a2%d7%9e%d7%a7%d7%94-%d7%9e%d7%92%d7%9c%d7%99%d7%9d-%d7%90%d7%aa-%d7%94%d7%9b%d7%95%d7%97%d7%95%d7%aa-%d7%a9/")}
                  className="p-2 rounded-xl bg-white/5 hover:bg-emerald-500/15 border border-white/10 hover:border-emerald-500/40 text-xs font-bold text-white transition-all cursor-pointer"
                >
                  <span className="block text-[10px] text-emerald-400 font-normal">שכבת</span>
                  <span>ח' צמיחה</span>
                ...
                <button
                  onClick={() => onNavigateToPage("course/%d7%97%d7%98%d7%91/%d7%a9%d7%9b%d7%91%d7%aa-%d7%98-%d7%91%d7%97%d7%99%d7%a8%d7%95%d7%aa-%d7%a2%d7%a6%d7%9e%d7%90%d7%95%d7%aa-%d7%95%d7%90%d7%97%d7%a8%d7%99%d7%95%d7%aa/")}
                  className="p-2 rounded-xl bg-white/5 hover:bg-emerald-500/15 border border-white/10 hover:border-emerald-500/40 text-xs font-bold text-white transition-all cursor-pointer"
                >
                  <span className="block text-[10px] text-emerald-400 font-normal">שכבת</span>
                  <span>ט' בוגרת</span>
            ...
            {/* HIGH SCHOOL HUB CARD */}
            <div className={`p-5 rounded-3xl border transition-all ${
              studentDivisionFilter === 'high' 
                ? 'bg-gradient-to-br from-violet-950/40 via-school-panel to-school-bg border-school-violet/60 shadow-xl' 
                : 'bg-school-panel/60 border-school-line/60 hover:border-school-violet/40'
            }`}>
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-2xl bg-school-violet/20 text-purple-300 flex items-center justify-center font-black text-sm">
                    י'-יב'
                  </div>
                  <div>
                    <h3 className="text-base font-black text-white">חטיבה עליונה (תיכון)</h3>
                    <p className="text-[11px] text-school-muted">11 מגמות מדעיות וטכנולוגיות, בגרויות והכנה לצה"ל</p>
                  </div>
                </div>
                <button
                  onClick={() => onNavigateToPage('students-high')}
                  className="text-[11px] font-black px-2.5 py-1 rounded-lg bg-school-violet/20 hover:bg-school-violet/30 text-purple-200 border border-school-violet/40 transition-all cursor-pointer flex items-center gap-1"
                >
                  <span>מרחב החטיבה</span>
                  <ChevronRight className="w-3 h-3 rotate-180" />
                </button>
              ...
              <div className="grid grid-cols-3 gap-2 pt-1 text-center">
                <button
                  onClick={() => onNavigateToPage("course/%d7%97%d7%98%d7%a2-2/%d7%a4%d7%95%d7%a1%d7%98-%d7%90%d7%91%d7%90-%d7%a9%d7%9b%d7%91%d7%aa-%d7%99/")}
                  className="p-2 rounded-xl bg-white/5 hover:bg-school-violet/20 border border-white/10 hover:border-school-violet/40 text-xs font-bold text-white transition-all cursor-pointer"
                >
                  <span className="block text-[10px] text-purple-300 font-normal">שכבת</span>
                  <span>י' כניסה למגמות</span>
                </button>
                <button
                  onClick={() => onNavigateToPage("course/%d7%97%d7%98%d7%a2-2/%d7%90%d7%91%d7%90-%d7%a9%d7%9b%d7%91%d7%aa-%d7%99%d7%90/")}
                  className="p-2 rounded-xl bg-white/5 hover:bg-school-violet/20 border border-white/10 hover:border-school-violet/40 text-xs font-bold text-white transition-all cursor-pointer"
                >
                  <span className="block text-[10px] text-purple-300 font-normal">שכבת</span>
                  <span>יא' בגרויות</span>
                </button>
                <button
                  onClick={() => onNavigateToPage("course/%d7%97%d7%98%d7%a2-2/%d7%a9%d7%9b%d7%91%d7%aa-%d7%99%d7%91/")}
                  className="p-2 rounded-xl bg-white/5 hover:bg-school-violet/20 border border-white/10 hover:border-school-violet/40 text-xs font-bold text-white transition-all cursor-pointer"
                >
                  <span className="block text-[10px] text-purple-300 font-normal">שכבת</span>
                  <span>יב' צה"ל וסיום</span>
                </button>
              </div>
            </div>
          </div>
