            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="p-2.5 rounded-xl bg-school-cyan/10 text-school-cyan border border-school-cyan/20 group-hover:scale-105 transition-transform">
                  {getIcon(major.icon || (major as any).iconName)}
                </div>
                <div className="flex items-center gap-1.5">
                  {major.targetGrades && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-school-panel2 text-school-muted border border-school-line">
                      {major.targetGrades}
                    </span>
                  )}
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-school-violet/15 text-school-violet border border-school-violet/30">
                    {major.hoursPerWeek || (major as any).units || '5 יח"ל'}
                  </span>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between gap-2">
                  ...
                  {!major.isActive && (
                    <span className="px-2 py-0.5 rounded-md text-[11px] font-black bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 shrink-0">
                      בקרוב
                    </span>
                  )}
                </div>
              ...
              {((major.highlights && major.highlights.length > 0) || ((major as any).keyTopics && (major as any).keyTopics.length > 0)) && (
                <div className="space-y-1 pt-1 border-t border-school-line/40">
                  {(major.highlights || (major as any).keyTopics).filter(Boolean).slice(0, 2).map((h: string, idx: number) => (
              ...
              {(major.coordinatorName || major.contactPerson) && (
                <div className="flex items-center gap-1.5 text-[10px] text-school-muted/80 pt-1">
                  <User className="w-3 h-3 text-school-cyan/70 shrink-0" />
                  <span className="truncate">רכז/ת: {major.coordinatorName || major.contactPerson}</span>
                </div>
              )}
            </div>
