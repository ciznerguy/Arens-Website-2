...
export default function App() {
    ...
    const handleUrlRouteSync = () => {
      try {
        ...
        if (hash === 'home-page-1' || pageParam === 'home-page-1' || window.location.pathname.endsWith('/home-page-1')) {
          ...
          setActiveTab('majors');
          setSelectedInternalPageUrl(null);
        } else if (hash === 'newsletter' || pageParam === 'newsletter' || hash.includes('מידעון') || hash.includes('%d7%9e%d7%99%d7%93%d7%a2%d7%95%d7%9f')) {
          setActiveTab('newsletter');
          setSelectedInternalPageUrl(null);
        } else if (hash.startsWith('course/')) {
          if (hash.includes('מידעון') || hash.includes('%d7%9e%d7%99%d7%93%d7%a2%d7%95%d7%9f')) {
            setActiveTab('newsletter');
            setSelectedInternalPageUrl(null);
          } else {
  ...
  const NAV_ITEMS = [
    ...
    },
    {
      ...
      tab: "students",
      sub: [
        { t: "חטיבת נעורים", url: "students-junior" },
        { t: "חטיבה עליונה", url: "students-high" }
      ]
    },
    {
      t: "להורים",
  ...
  const EXTERNAL_LINKS = [
    ...
    { t: "השאלת ספרים תשפ\"ז", url: "course/%d7%a4%d7%a8%d7%95%d7%99%d7%a7%d7%98-%d7%94%d7%a9%d7%90%d7%9c%d7%aa-%d7%a1%d7%a4%d7%a8%d7%99%d7%9d-%d7%aa%d7%a9%d7%a4%d7%95/" },
  ...
  const handleNavClick = (item: any) => {
    ...
    } else if (item.url === 'newsletter' || item.tab === 'newsletter' || (item.url && (item.url.includes('מידעון') || item.url.includes('%d7%9e%d7%99%d7%93%d7%a2%d7%95%d7%9f')))) {
      setSelectedInternalPageUrl(null);
      setActiveTab('newsletter');
    ...
    } else if (item.tab === 'majors' || item.url === 'majors') {
      setSelectedInternalPageUrl(null);
      setActiveTab('majors');
            ...
            {currentNavItems.map((item, idx) => (
              ...
              >
                ...
                {item.sub && hoveredNavIndex === idx && (
                  <div 
                    className="absolute top-full right-0 pt-2 min-w-[280px] max-w-[340px] z-50 animate-fade-in"
                    onMouseEnter={() => handleNavMouseEnter(idx)}
                    onMouseLeave={handleNavMouseLeave}
                  >
                    <div className="p-2 rounded-2xl bg-[#131d2e]/98 border border-school-line/90 shadow-2xl backdrop-blur-2xl space-y-0.5 max-h-[75vh] overflow-y-auto pr-1">
                      {item.sub.map((subItem: any, subIdx) => {
                        if (subItem.isHeader) {
                          return (
                            <div 
                              key={subIdx} 
                              className="px-3 py-1.5 mt-2 first:mt-0 text-[10px] font-black text-school-cyan uppercase tracking-wider bg-school-cyan/10 rounded-lg border border-school-cyan/25 flex items-center gap-1.5"
                            >
                              <span>{subItem.t}</span>
                            </div>
                          );
                        ...
                        return (
                          <button
                            ...
                            className="w-full text-right px-3 py-2 rounded-xl text-xs font-semibold text-school-muted hover:text-white hover:bg-gradient-to-r hover:from-school-cyan/20 hover:to-school-violet/20 transition-all duration-150 cursor-pointer flex items-center justify-between group"
                          >
                            <span className="group-hover:text-white transition-colors">{subItem.t}</span>
              ...
              <div className="p-4 space-y-4">
                {currentNavItems.map((item, idx) => (
                  <div key={idx} className="space-y-1.5 border-b border-school-line/30 pb-3 last:border-0 last:pb-0">
                    ...
                    {item.sub && (
                      <div className="grid grid-cols-1 gap-1 pr-3 border-r border-school-line/60 space-y-0.5">
                        {item.sub.map((subItem: any, subIdx) => {
                          if (subItem.isHeader) {
                            return (
                              <div
                                key={subIdx}
                                className="pt-2.5 pb-1 text-[10px] font-black text-school-cyan uppercase tracking-wider first:pt-0"
                              >
                                {subItem.t}
                              </div>
                            );
                          ...
                          return (
                            <button
                              key={subIdx}
                              onClick={() => handleNavClick(subItem)}
                              className="text-right py-1.5 text-xs text-school-muted hover:text-white transition-all flex items-center justify-between group"
                            >
                              <span className="group-hover:text-school-cyan transition-colors">{subItem.t}</span>
                              {subItem.externalUrl && <ExternalLink className="w-2.5 h-2.5 text-school-cyan/60 shrink-0" />}
                            </button>
                          );
                        })}
                      </div>
            ...
            >
              <InternalPageViewer 
                ...
                onNavigateToPage={(url) => {
                  if (url === 'contact') {
                    setActiveTab('contact');
                  } else if (url === 'newsletter' || url.includes('מידעון') || url.includes('%d7%9e%d7%99%d7%93%d7%a2%d7%95%d7%9f')) {
                    setSelectedInternalPageUrl(null);
                    setActiveTab('newsletter');
                    window.location.hash = 'newsletter';
                  } else if (url === 'majors') {
                    ...
                    setActiveTab('majors');
                    window.location.hash = 'majors';
                  } else if (url.startsWith('major-')) {
                    setSelectedMajorId(url.replace('major-', ''));
                    setSelectedInternalPageUrl(null);
                    setActiveTab('major-page');
